...
|---|---|---|
| [`patches/`](../../.py/klipper/patches/) | Replaces an existing Klipper module or host helper; original retained as `.bak` | Core/config parsing and G-code handling; selected `extras` modules; the matched adaptive-PA Python/CFFI modules and `c_helper.so`. |
