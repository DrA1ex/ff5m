...
|---|---|---|---|
| Project/product entry | [`README.md`](../README.md) | `docs/INSTALL.md`, `docs/PRINTING.md` | Scope, supported devices, safety posture, endpoints. |
| Early boot and recovery guard | [`.shell/S00init`](../.shell/S00init) | `.shell/boot/recovery.sh`, `.shell/init-main.sh`, `.shell/boot/boot_mode.sh` | Durable recovery decisions, hard stock fallback, and confirmed normal-init readiness. Runtime reload is not part of this boundary. |
| Early recovery | [`.shell/boot/recovery.sh`](../.shell/boot/recovery.sh) | `.py/recovery.py`, `.shell/boot/stock_identity.sh`, `.shell/boot/install-image.sh`, `.shell/commands/zcheck.sh` | Fail-open boot gate, temporary recovery resources, touch UI actions, image downloads, verification, and canonical firmware hand-off. |
| Chroot runtime and service lifecycle | [Chroot environment and web runtime](workflows/chroot-and-web-runtime.md) | `.shell/init-main.sh`, `.shell/S99root`, `.root/start.sh`, `.root/S65moonraker`, `.root/S70httpd` | Shared mounts, service ownership, command/tool boundary, and Moonraker/UI topology. |
...
| Klipper patch/plugin overlay | [`.py/klipper/`](../.py/klipper/) | [Built-in Klipper patching](workflows/klipper-patching.md), [Forge-X Klipper extensions](workflows/klipper-extensions.md), `.shell/init-main.sh`, `.shell/uninstall.sh` | Symlinked stock-module replacements with `.bak` rollback plus configured extra plugins; not a full source fork. |
...
- **Support/compatibility:** `INSTALL.md`.
