...
- **`block_cloud`**: Blocks the stock FlashForge cloud / telemetry endpoints (Flash Studio / MQTT, VoxelShare, stock OTA, NetEase IM) if set to `1`. Default `0` (off) — leave disabled if you use FlashForge cloud, video streaming or model sharing. Blocking is done at name-resolution level (`/etc/hosts`) only, so it does not affect DHCP or network init. See [FIRMWARE_5x_COMPAT.md](/docs/FIRMWARE_5x_COMPAT.md#hosts-blocked) for the host list and what each one does. Full air-gap additionally requires router-level blocking of AlibabaCloud OSS upload URLs.  
