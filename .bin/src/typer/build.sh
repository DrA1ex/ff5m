...
# Build typer for the native printer environment (soft-float ARM EABI).
#
...
Build the 'typer' native printer utility as a soft-float ARM EABI binary.
...
# --------------------------------------------------------------------------
echo "==> Configuring typer (${TOOLCHAIN_TRIPLET}, ${BUILD_TYPE})"
cmake \
    -S "$SCRIPT_DIR" \
    -B "$BUILD_DIR" \
